...
Here are the main differences between this version and the previous version (```alto v0.0.7```):
  <Accordion title="Additional EC2 protocols">      
    ...
    specify whether they want to use Amazon Systems Manager to manage their instance. AWS
    Systems Manager requires fewer resources to instantiate and execute commands on an EC2
    instance.

    This can be specified via the `protocol` key within an EC2 agent's `infra` configuration.
  </Accordion>
